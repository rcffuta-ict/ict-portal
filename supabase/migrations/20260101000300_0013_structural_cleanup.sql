...
--
-- THREE DEAD RLS POLICIES HAVE TO GO FIRST, or the drop fails outright:
--
--     ERROR: cannot drop column category of table leadership_positions
--            because other objects depend on it
--
-- This is the same shape as the question_flags mistake documented above — a
-- dependency invisible to a grep of src/, found only by actually running the
-- migration against production's schema. It was: rehearsing this chain on a throwaway
-- Postgres is what surfaced it, and every environment would have hit it.
--
-- The policies are vestigial. They date from before Supabase Auth was retired and test
-- `auth.uid()`, which is NULL for the service role — and every portal table is
-- service-role-only with RLS FORCED and no policies (0001's posture, re-asserted in
-- section 5 below). So they grant nothing to anyone today, and have not since 0001.
--
-- Dropped explicitly rather than with DROP COLUMN ... CASCADE: CASCADE would remove
